...
def gini(y: np.ndarray) -> float:
    ...
    """
    _, label_cnt = np.unique(y, return_counts=True)
    gini_index = 1 - np.sum((label_cnt / len(y)) ** 2)
    return gini_index


def weighted_impurity(y_left: np.ndarray, y_right: np.ndarray) -> tp.Tuple[float, float, float]:
    """
...
def create_split(feature_values: np.ndarray, threshold: float) -> tp.Tuple[np.ndarray, np.ndarray]:
    ...
    idx_range = np.arange(feature_values.shape[0])
    left_idx = idx_range[left_mask]
    right_idx = idx_range[~left_mask]
...
class MyDecisionTreeNode:
    ...
    def _best_split(self, X: np.ndarray, y: np.ndarray):
        ...
        for feature in features:
            ...
            for threshold in thresholds:
                current_left_idx, current_right_idx = create_split(
                    current_feature_values, threshold
                )
                current_y_left, current_y_right = y[current_left_idx], y[current_right_idx]
                current_weighted_impurity, current_left_impurity, current_right_impurity = (
                    weighted_impurity(current_y_left, current_y_right)
                )
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        if (
            ...
            or X.shape[0] < self._meta.min_samples_split
            or self._impurity == 0
        ):
            self._node_type = NodeType.TERMINAL

            cls_cnt = np.zeros(self._meta._n_classes, dtype=np.float64)
            unique_cls, cnt = np.unique(y, return_counts=True)
            cls_cnt[unique_cls] = cnt

            self._predicted_class = np.argmax(cls_cnt)  # choose most common class
            self._class_proba = (
                cls_cnt / y.shape[0]
        ...
        self._left_subtree = MyDecisionTreeNode(
            meta=self._meta, depth=self._depth + 1, impurity=left_imp  # adjust depth
        ).fit(
            X[left_idx], y[left_idx]  # choose proper data to fit
        )
        self._right_subtree = MyDecisionTreeNode(
            meta=self._meta, depth=self._depth + 1, impurity=right_imp  # adjust depth
        ).fit(
    ...
    def predict(self, x: np.ndarray):
        ...
        if x[self._feature_id] <= self._threshold:  # look for an answer recursively
...
class MyRandomForestClassifier:
    ...
    def _bootstrap_sample(self, X: np.ndarray, y: np.ndarray):
        ...
        indices = np.random.choice(
            X.shape[0], X.shape[0], replace=True  # True?
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        for estimator in self.estimators:
            estimator.fit(*self._bootstrap_sample(X, y))
        self._is_trained = True
        return self
    ...
    def predict_proba(self, X: np.ndarray):
        ...
        probas = np.mean([estimator.predict_proba(X) for estimator in self.estimators], axis=0)
        return probas
    ...
    def predict(self, X):
        ...
        return y_pred  # most probable class for each object
...
class Logger:
                ...
                if file.lower().endswith(".json"):
                    ...
                    with open(os.path.join(root, file), "r") as f:
                        try:
                            ...
                            records.append({"id": uuid, **logged_data})  # FIXME: _append?
    ...
    class Run:
        ...
        def __init__(self, name, storage, path):
            ...
            self._path = path
            self._storage._append(pd.Series(name=name))
...
def load_predictions_dataframe(filename, column_prefix, index):
    with open(filename, "rb") as file:
        ...
        dataframe = pd.DataFrame(
            data, columns=[f"{column_prefix}_{i}" for i in range(data.shape[1])], index=index
        )
        return dataframe
...
class ExperimentHandler:
    ...
    def score_test(self, estimator, metrics, run, test_data=None):
        ...
        test_scores = {name : _score(estimator, test_data, self.y_test, m, {}) for name, m in metrics.items()}
    ...
    def score_cv(self, estimator, metrics, run):
        ...
        for key, value in cross_val_results.items():
            if key.startswith("test_"):
                ...
                run.log_values({metric_name + "_mean": mean_score, metric_name + "_std": std_score})
    ...
    def get_stacking_predictions(self, run_names):
        ...
        for run_name in run_names:
            train_filename = (
                self.logger.path / run_name / ExperimentHandler.stacking_prediction_filename
            )
            train_dataframes.append(
                load_predictions_dataframe(
                    filename=train_filename, column_prefix=run_name, index=self.X_train.index
                )
            )
            test_filename = (
                self.logger.path / run_name / ExperimentHandler.test_stacking_prediction_filename
            )
            test_dataframes.append(
                load_predictions_dataframe(
                    filename=test_filename, column_prefix=run_name, index=self.X_test.index
                )
            )
